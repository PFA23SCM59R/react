  ...
  */
  React.useEffect(async () => {
    ...
    const requestOptions2 = {
      ...
      // Append the repository key to request body
      body: JSON.stringify({ repos: repositories.map(repo => repo.key) }),
    };
    /*
    ...
    */
    await fetch("/api/linechart",requestOptions2)
      .then((res) => res.json())
      .then(
    ...
    await fetch("/api/github", requestOptions)
      .then((res) => res.json())
      .then(
        // On successful response from flask microservice
        (result) => {
        ...
        {loading ? (
          ...
          <div>
            <LineChart
              ...
              data={reposLineChartData}
            />
            <Divider
              sx={{ borderBlockWidth: "3px", borderBlockColor: "#FFA500" }}
            />
            <BarCharts
              title={`Weekly Closed Issues for ${repository.value} in last 2 month`}
            ...
            />
            <BarCharts
              title={`Total Forks for ${repository.value} in last 2 month`}
              data={[githubRepoData?.forkCount]}
              yAxis={"Forks"}
            />
            <BarCharts
              title={`Total Stars for ${repository.value} in last 2 month`}
